...
│  │  └─ feature_request.yml
│  ├─ instructions/
│  │  └─ reactjs.instructions.md    # Scoped guidance for React files
│  ├─ copilot-instructions.md       # Repo-wide Copilot guidance
│  ├─ CODEOWNERS
│  ├─ SECURITY.md
│  └─ dependabot.yml
├─ src/                              # Current app source (frontend)
...
├─ V1/                               # Previous full app scaffold (frontend + API)
│  ├─ index.html                     # HTML entry
│  ├─ vite.config.js                 # Vite configuration
│  ├─ package.json                   # Dependencies & scripts for V1
│  ├─ public/
│  │  └─ README.txt
│  ├─ server/
│  │  ├─ index.js                    # Azure OpenAI relay (non-streaming)
│  │  └─ .env.example                # Example env vars (server)
│  ├─ dist/                          # Production build output (V1)
│  ├─ .env.example
│  └─ logo.png
└─ README.md
```

Note: The `src/` directory contains the current app code. The `V1/` folder preserves the earlier full-stack layout (including the Express API) for reference and reuse.
